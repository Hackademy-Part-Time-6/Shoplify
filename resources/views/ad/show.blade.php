                ...
                <div>
                    ...
                    @if (auth()->user()->favorites->contains('ad_id', $ad->id))
                        <form action="{{ route('favorites.remove', $ad) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn show-btn-delete m-2" data-ad-id="{{ $ad->id }}">{{ __('Eliminar de favoritos') }}<i class="fa-solid fa-heart-crack ms-1"></i></button>
                        </form>
                    @else
                        <form action="{{ route('favorites.add', $ad) }}" method="POST" class="d-inline">
                            @csrf
                            <button type="submit" class="btn show-btn m-2" data-ad-id="{{ $ad->id }}">{{ __('Guardar como favorito') }}<i class="fa-solid fa-heart ms-1"></i></button>
                        </form>
                    @endif
                @endif
                </div>
